...
// 2-stage final decrypt round (round 0) — no InvMixColumns.
//
// Stage 1: InvShiftRows (free) + InvSubBytes (GF comb) → sb_reg
// Stage 2: AddRoundKey → state_out  (no InvMixColumns in round 0)

module aes_dec_round_final_fast (
    input  wire        clk,
    ...
    // STAGE 2 — no InvMixColumns
    always @(posedge clk)
        state_out <= sb_reg ^ round_key;
