`timescale 1ns/1ps
  ...
  module aes128_encrypt (
      ...
      // ----------------------------------------------------------------
      // Round key wires (combinational output of aes_key_exp)
      // ----------------------------------------------------------------
      wire [127:0] rk1, rk2, rk3, rk4, rk5;
      wire [127:0] rk6, rk7, rk8, rk9, rk10;

      // ----------------------------------------------------------------
      // Round key pipeline registers (delay keys to match data pipeline)
      // ----------------------------------------------------------------
      reg [127:0] rk1r, rk2r, rk3r, rk4r, rk5r;
      reg [127:0] rk6r, rk7r, rk8r, rk9r, rk10r;

      // ----------------------------------------------------------------
      // Data pipeline wires (output of each round module)
      // ----------------------------------------------------------------
      wire [127:0] st1, st2, st3, st4, st5;
      wire [127:0] st6, st7, st8, st9, st10;

      // ----------------------------------------------------------------
      // Key expansion - 10 instances, each generates next round key
      // ----------------------------------------------------------------
      ...
      // ----------------------------------------------------------------
      // Encryption pipeline
      // Round 1 input = stage0 XOR key0  (initial AddRoundKey)
      // Rounds 2-9 = normal rounds
      // Round 10   = final round (no MixColumns)
      // ----------------------------------------------------------------
      aes_enc_round r1 (
          .clk(clk), .state_in(stage0 ^ key0), .round_key(rk1r), .state_out(st1)
      );
      aes_enc_round r2 (
          .clk(clk), .state_in(st1), .round_key(rk2r), .state_out(st2)
      );
      aes_enc_round r3 (
          .clk(clk), .state_in(st2), .round_key(rk3r), .state_out(st3)
      );
      aes_enc_round r4 (
          .clk(clk), .state_in(st3), .round_key(rk4r), .state_out(st4)
      );
      aes_enc_round r5 (
          .clk(clk), .state_in(st4), .round_key(rk5r), .state_out(st5)
      );
      aes_enc_round r6 (
          .clk(clk), .state_in(st5), .round_key(rk6r), .state_out(st6)
      );
      aes_enc_round r7 (
          .clk(clk), .state_in(st6), .round_key(rk7r), .state_out(st7)
      );
      aes_enc_round r8 (
          .clk(clk), .state_in(st7), .round_key(rk8r), .state_out(st8)
      );
      aes_enc_round r9 (
          .clk(clk), .state_in(st8), .round_key(rk9r), .state_out(st9)
      );
      aes_enc_round_final r10 (
          .clk(clk), .state_in(st9), .round_key(rk10r), .state_out(st10)
      );
