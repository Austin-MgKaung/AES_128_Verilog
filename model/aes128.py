...
def encrypt_block_algorithmic(key: bytes, pt: bytes, first: bool = False) -> bytes:
    """
    ...
    key_expanded = key_expansion_algorithmic(key)
...
def encrypt_block_LUTS(key: bytes, pt: bytes, first: bool = False) -> bytes:
    """
    Encrypt one 16-byte block with a 16-byte AES-128 key.
    Return: 16-byte ciphertext.
    """
...
def decrypt_block_algorithmic(key: bytes, ct: bytes, first: bool = False) -> bytes:
    """
...
def decrypt_block_LUTS(key: bytes, ct: bytes, first: bool = False) -> bytes:
    """
    Decrypt one 16-byte block with AES-128/192/256 depending on key length.
    Return: 16-byte plaintext.
    """
